...
function LoginDesign() {
  ...
  const handleSubmitSign = async (event) => {
    ...
    try {
      const response = await axios.post("http://localhost:4040/login-design",JSON.stringify({emailSign,passwordSign}),{
    ...
    const handleSubmitLog = async (event) => {
      event.preventDefault();
      try {
        const response = await axios.post("http://localhost:4040/auth",JSON.stringify({emailLog,passwordLog}),{
          headers : { 'Content-Type' : 'application/json'},
          withCredentials: true
        }
        );
    ...
    // axios
    //   .post("http://localhost:4040/login-design", { email, password })
    //   .then((response) => {
