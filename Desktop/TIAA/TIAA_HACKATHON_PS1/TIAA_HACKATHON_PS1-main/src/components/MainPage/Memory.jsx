...
const Memory = ({ memory: { title, text, subtitle, img, experience } }) => {
  ...
  return (
    <div id="about">
      <div className="relative my-16 md:mb-7 bg-white">
        <div className="travigo-container flex items-center justify-between gap-16 xl:gap-9 lg:flex-col-reverse">
          <div className="flex items-center justify-start lg:justify-center w-full max-w-md">
            <img src={img} alt="memory/img" className="object-fill" />
          </div>
          <div className="grid items-center w-full max-w-2xl lg:text-center">
            ...
            <div className="grid items-center grid-cols-3 gap-7 xl:gap-3">
              {experience?.map((val, i) => (
                <div
                  key={i}
                  className="bg-gradient-to-b from-blue-400 to-blue-400 shadow-lg shadow-blue-300 flex items-center justify-center flex-col py-7 px-5 xl:p-5 rounded-lg text-slate-900 filter cursor-pointer hover:scale-105 transition-all duration-400"
                >
                  <h1 className="text-3xl xl:text-2xl sm:text-xl font-bold drop-shadow-lg">
                    {val.number}
                  </h1>
                  <p className="text-md xl:text-base sm:text-sm xsm:text-xs font-medium">
                    {val.title}
                  </p>
                </div>
              ))}
